...
data class Character(
    ...
    // TODO (possibly) if you wold rather use the array versions, comment out the map versions
    // uncomment the array versions, and you will also need to track down wherever the maps
    // are currently being used and swap them out for the arrays
    ...
    // visual elements for the character
    ...
    fun hasValidTextFields(): Boolean {
        return !name.isEmpty() && !charClass.isEmpty() && !description.isEmpty()
    }

    fun arePointsAllSpent(): Boolean {
    ...
    fun withUpdatedStat(statName: String, delta: Int, maxPoints: Int): Character {
        ...
        var totalPts = totalPoints
        if (delta > 0 && totalPts >= maxPoints) return this
